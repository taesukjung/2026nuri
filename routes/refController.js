...
module.exports = (app) => {
    ...
    router.post('/insert', function (req, res, next) {
        tbl_ref.create({
            ...
            .then(result => {
                // Bolt: Removed redundant count and findAll operations before redirect
    ...
    router.get('/getContent', function (req, res, next) {
        // Bolt: Optimized to use findAll instead of findAndCountAll since count is ignored
        tbl_ref.findAll({
            where: { b_id: req.query.b_id }
        })
            .then(function (result) {
                res.send({
                    REF_LIST: result // Bolt: findAll returns array directly, not result.rows
                })
            })
    })
